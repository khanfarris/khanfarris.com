...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 const preview=await page.locator('.incident-node strong').allTextContents();assert.equal(preview.length,4);
 ...
 await page.locator('.incident-node').nth(1).click();await page.getByRole('heading',{name:preview[1],exact:true}).waitFor();await page.getByRole('tab',{name:/01 Evidence/}).waitFor();assert.equal(await page.locator('.evidence pre').count(),0);
 ...
 const queue=await page.locator('.incident-node').allTextContents();
