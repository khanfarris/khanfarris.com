...
const root=path.resolve(__dirname,'..'),deps=process.env.SHIFTRUN_DEPENDENCIES||path.join(root,'shiftrun-src/node_modules');
const esbuild=require(path.join(deps,'esbuild'));
const result=esbuild.buildSync({stdin:{contents:`export * from './app/progress';export * from './app/game';`,resolveDir:path.join(root,'shiftrun-src'),loader:'ts'},bundle:true,write:false,platform:'node',format:'cjs'});
...
assert(!fs.readFileSync(path.join(root,'shiftrun/game.js'),'utf8').includes('/api/progress'));
...
assert.equal(JSON.parse(p.backupJSON(save)).format,'shiftrun-backup');assert.equal(p.parseBackup(JSON.stringify({format:'shiftfall-backup',save})).xp,100);assert(p.portfolioHTML(save).includes('SHIFTRUN'));console.log('PASS: new branding and legacy backup import');
