(() => {
  ...
  if(title){
    ...
    const meta=document.createElement('p');meta.className='reading-meta';meta.textContent=Math.ceil(words/220)+' min read / FIELD INVESTIGATION';title.after(meta);
    const headings=Array.from(main.querySelectorAll('h2'));
    const toc=document.createElement('details');toc.className='toc';
    const summary=document.createElement('summary');summary.textContent='In this investigation / '+headings.length+' sections';toc.append(summary);
    ...
    const related=document.createElement('aside');related.className='related-reading';const label=document.createElement('span');label.className='eyebrow';label.textContent='NEXT INVESTIGATION';const link=document.createElement('a');const lifx=location.pathname.includes('lifx');link.href=lifx?'samsung-tv-pentest.html':'lifx-pentest.html';link.textContent=(lifx?'pwning my TV':'pwning my light bulbs')+' ↗';related.append(label,link);const footer=main.querySelector('footer');if(footer)footer.before(related);else main.append(related);
  }
