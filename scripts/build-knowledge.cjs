...
<script src="knowledge-index.js?v=concepts-19" defer></script><script src="reading.js" defer></script><script src="lab.js?v=2931b52" defer></script><script src="knowledge.js" defer></script><script src="study-exercises.js?v=khanos-1" defer></script>
</head>`;
...
if(homepage.includes('id="khanos-root"')){
  ...
  const fallback=articles.map(a=>`<a class="fallback-note" href="${url(a.slug)}"><strong>${esc(a.title)}</strong><span>${esc(a.principle)}</span></a>`).join('\n');
...
console.log('Built '+articles.length+' active study notes, directory, search index, KhanOS content, and homepage.');
})();
