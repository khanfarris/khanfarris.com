...
(async()=>{await new Promise(r=>server.listen(0,'127.0.0.1',r));const browser=await chromium.launch({headless:true,channel:'msedge'});try{
 ...
 await page.getByRole('button',{name:'Shift 2 Locked preview',exact:true}).click();
 const preview=await page.locator('.shift-preview article h3').allTextContents();assert.equal(preview.length,4);
 assert.equal(await page.getByRole('tab').count(),0);
 assert.equal(await page.evaluate(()=>localStorage.getItem('khanfarris-shiftfall-v1')),before);
 await page.getByRole('button',{name:'Shift 1 Completed',exact:true}).click();
 await page.getByRole('button',{name:/Continue without upgrade/}).click();
 await page.getByRole('button',{name:'Shift 2 Active',exact:true}).waitFor();
 ...
 await page.reload();await page.getByRole('button',{name:'Shift 1 Completed',exact:true}).click();assert.equal(await page.getByRole('tab').count(),4);
 ...
 assert.equal(await page.getByRole('button',{name:'Shift 1 Completed',exact:true}).getAttribute('aria-pressed'),'true');
 await page.getByRole('button',{name:'Shift 2 Active',exact:true}).click();await page.getByRole('tab',{name:/02 Respond/}).click();assert.equal(await page.locator('.action-grid button:enabled').count(),0);
 await page.getByRole('button',{name:'Return to your progress',exact:true}).click();
 const legacy={...current};delete legacy.shiftHistory;
 await page.evaluate(data=>localStorage.setItem('khanfarris-shiftfall-v1',JSON.stringify(data)),legacy);await page.reload();await page.getByRole('button',{name:'Shift 1 Completed',exact:true}).click();
