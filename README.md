...
The background selector offers Orbit (the original), Helix, Ripple, Globe, and Vortex. The choice is remembered on this device and can be shared with `?background=helix` (or another background ID), alongside `theme` and a note hash. All designs follow the current signal color and the workspace motion controls, including reduced motion. `khanos-backgrounds.js` provides the fixed-size meshes and renderer; the desktop owns one animation loop and the menu previews stay still. Drag the empty desktop to rotate, scroll to zoom, or use one-finger rotation and two-finger pinch on touchscreens. With the constellation focused, arrow keys rotate, plus/minus zoom, and 0 resets; double-click or the selector’s Reset view button also restores the default view. Zoom stays between 35% and 250%. View adjustments last for the current page and survive palette/design changes. Manual controls work with motion paused or reduced, and gestures over windows retain their normal behavior.
...
- The OSI model note uses a compact seven-layer diagram with native expand/collapse sections. Each layer reveals individual protocols, devices, and concepts. Hover, keyboard focus, or tap shows a short definition and layer-placement explanation, including boundary cases. Native layer disclosure remains independent; `StudyExercises.mount()` binds the shared tooltip controller. `node scripts/test-study-tips.cjs` checks dismissal, keyboard/touch behavior, viewport positioning, and key interview-topic coverage.
- The FTP, Telnet, IMAP/IMAPS, POP3/POP3S, SNMP, SMTP/SMTPS, RPC/NetBIOS, HTTP/HTTPS, SSH, RDP, SMB, and LDAP/LDAPS concept notes include common server ports, layer explanations, and fictional prediction cases. Their `.protocol-lab` activities use native `<details>` groups: opening a case reveals a three-step diagram and a practical explanation, with no additional script or saved state. Keep these cases in `knowledge-content.json`; their shared responsive styles live in `knowledge.css`. Collapsed reading links inside the OSI layers connect to the full protocol notes.
...
For future lab writeups, omit reading-time estimates. Keep tutorial attribution general (for example, “a tutorial I followed”). Do not link to or identify the specific video, tutorial title, creator, or channel. Never mention transcripts in lab copy. Image captions should contain only the description, without “From my saved lab screenshots:” or similar ownership lead-ins. Elsewhere, refer to screenshots as “my saved lab screenshots.” Use the exact question wording when the user supplies the lab questions. Retain useful official product documentation links and accurate distinctions between reference material and observed results.
...
Before publishing lab material, check supplied READMEs, text, and images for credentials and sensitive information. Omit usernames, passwords, unlock passwords, API keys, tokens, and other secrets from public content, including provided training-lab credentials. Keep useful setup instructions without reproducing the login details.

Do not add an introductory summary paragraph describing completion of the lab or previewing the investigation. Do not add an opening paragraph explaining screenshot highlights, enlargement, or reference-checked evidence. Keep the title/byline, any supplied README, and the actual walkthrough; the closing red Recap remains part of the format.

For every BTL writeup, the name after `BTL /` in the opening label must exactly match that writeup's title on `btl.html`, including numbers, spelling, and capitalization. Wrap the name in `.btl-writeup-name` to preserve its capitalization inside the eyebrow label.
